<script>
	...
	function buildRows(payload) {
		return selectedRates.map((currency) => ({
			currency,
	...
	function buildChartConfig(chartRows) {
		return {
			...
			xAxis: { categories: chartRows.map((row) => row.currency), title: { text: null } },
			yAxis: { min: 0, title: { text: 'Tipo de cambio' } },
			legend: { enabled: false },
			credits: { enabled: false },
			series: [{ name: 'Rate', data: chartRows.map((row) => row.rate), color: '#5d6f96' }]
		};
	}
