<script>
	...
	const API_URL = 'https://sos2526-20-stable.onrender.com/api/v2/wool-stats?limit=30';
	const tableColumns = [
		{ key: 'label', label: 'registro' },
		{ key: 'quantity', label: 'cantidad' },
		{ key: 'value', label: 'valor' }
	];
	const quantityKeys = ['qty', 'netwgt', 'grosswgt'];
	const valueKeys = ['primaryvalue', 'cifvalue', 'fobvalue'];
	...
	function maxValue(item, keys) {
		return Math.max(...keys.map((key) => numberValue(item[key])));
	}
	...
	function buildRows(payload) {
		...
		return sourceRows
			.map((item) => ({
				...
				quantity: maxValue(item, quantityKeys),
				value: maxValue(item, valueKeys)
			}))
	...
	function buildChartConfig(chartRows) {
		return {
			...
			series: [
				...
				{ name: 'Cantidad', data: chartRows.map((row) => row.quantity), color: '#51643b', yAxis: 1 }
			]
		};
	}
...
<main class="page">
	<header class="header">
		<div>
			<h1>Wool Stats</h1>
			<p>
				Comparativa entre cantidad comercializada y valor comercial con datos reales de comercio de
				lana.
			</p>
		</div>
		<div class="meta-grid">
	...
	<section class="content">
		...
		{:else}
			<p class="status">
				La API de lana respondió, pero no devolvió registros con cantidad y valor positivos.
			</p>
		{/if}
	</section>
</main>
