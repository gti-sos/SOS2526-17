<script>
	...
	async function loadData() {
		...
		try {
			const response = await fetch(withCacheBust('/api/v1/integrations-proxy/drinking-water-services?limit=20'));
...
<main class="page">
	...
	<section class="content">
				...
				<table>
					<thead>
						<tr>
							...
							<th>code</th>
							<th>year</th>
							<th>wat_bas_pop_residence_urban</th>
						</tr>
					</thead>
					<tbody>
		...
		{:else}
			<p class="status">La API respondió en JSON pero no devolvió filas con los parámetros actuales.</p>
		{/if}
	</section>
</main>
