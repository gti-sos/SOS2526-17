<script>
	...
	const API_URL = 'https://sos2526-10.onrender.com/api/v2/pandemics?limit=40';
	const tableColumns = [
		{ key: 'label', label: 'registro' },
		{ key: 'total', label: 'total' },
		{ key: 'cholera', label: 'cholera' },
		{ key: 'malaria', label: 'malaria' },
		{ key: 'hiv', label: 'hiv_aids' },
		{ key: 'tuberculosis', label: 'tuberculosis' }
	];
	const diseaseKeys = ['cholera', 'malaria', 'hiv_aids', 'tuberculosis', 'rabies'];
	...
	function buildRows(payload) {
		return (Array.isArray(payload) ? payload : [])
			.map((item) => ({
				...
				total: diseaseKeys.reduce((sum, key) => sum + numberValue(item[key]), 0)
			}))
